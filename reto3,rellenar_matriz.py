...
"""
This program creates a matrix with the data you input: rows, columns, 
and values for each element.
"""
...
def prompt_option_number():
    """
    It doesn't take any arguments, prompts the user for an option among 
    three previously given options, and returns the chosen option in 
    INTEGER format.
    """
...
def prompt_option():
    """
    It doesn't take any arguments,promts the user one option among 
    ´Yes´ or ´no´ previusly given options, and returns the shosen option
    """
    while True:
...
def continue_program():
    """
    Reduce the code when interacting with the user, without accepting 
    any parameters as arguments."
    """
...
def presentation():
    """
    Show the program's presentation, it does not take any parameters, 
...
def built_matriz():
    try:
        rows=int(input("Please, ENTER any number of ROWS to built the "
                       " matriz: "))
        columns=int(input("please, ENTER any number of COLUMNS to built "
                         "the matriz: "))
    except ValueError:
...
while True:
    ...
    if matriz == None and option == 1:
        ...
        print("You already have a matriz built, You want to rewrite the existing"
              " one?")
